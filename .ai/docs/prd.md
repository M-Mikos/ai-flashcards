...
3. Zarządzanie fiszkami użytkownika w ramach widoku listy "Moje fiszki"
   - wyświetlanie zapisanych fiszek
   - edycja zapisanych fiszek
   - usuwanie zapisanych fiszek
4. Manualne tworzenie pojedynczych fiszek (formularz z polami przód / tył) w ramach widoku listy "Moje fiszki".
5. Nauka fiszek wg harmonogramu w ramach widoku "Sesja nauki" - integraja z gotowym algorytmem:
...
6. Prosty system kont i uwierzytelniania: rejestracja, logowanie, reset hasła (e-mail + token), usunięcie konta i powiązanych fiszek.
7.  Statystyki: zapis źródła wygenerowania fiszki (ai/ai+edycja/manulanie) w tabeli fiszek, podstawowe logi błędów sesji generacji.
8.  Responsywny, dostępny, prosty interfejs mobile-first.
...
| US-011 | Dostęp tylko po zalogowaniu | Jako niezalogowany użytkownik nie powinienem mieć dostępu do fiszek ani generowania. | 1) Próba wejścia na chronione strony przekierowuje na stronę logowania. |
